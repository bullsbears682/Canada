...
exports.OntarioEnergyBoardDataSource = void 0;
/**
 ...
 */
class OntarioEnergyBoardDataSource {
    constructor(apiKey) {
        this.name = 'Ontario Energy Board';
        this.baseUrl = 'https://api.oeb.ca';
     ...
     */
    async getProvincialUtilityRates() {
     ...
     */
    async getElectricityRates(rateClass = 'residential') {
     ...
     */
    async getNaturalGasRates(rateClass = 'residential') {
     ...
     */
    async getUtilityProviders(region) {
     ...
     */
    async getRateComparison(utilityType) {
    ...
    async makeRequest(endpoint, params) {
        ...
        try {
            ...
            if (data.error) {
                throw new Error(`OEB API error: ${data.error.message || data.error}`);
            }
            return data;
        }
        catch (error) {
     ...
     */
    transformUtilityRatesResponse(response, location) {
     ...
     */
    transformProvincialRatesResponse(response) {
     ...
     */
    transformElectricityRatesResponse(response) {
        const rates = response.rates || {};
        return {
            residential: {
     ...
     */
    transformNaturalGasRatesResponse(response) {
        const rates = response.rates || {};
        return {
            residential: {
                rate: rates.residential?.rate || 0,
     ...
     */
    transformProvidersList(providers) {
        return providers.map((provider) => ({
            name: provider.name || 'Unknown',
            type: provider.type || 'public',
            rates: this.transformProviderRates(provider.rates || {}),
     ...
     */
    transformComparisonResponse(response) {
...
exports.OntarioEnergyBoardDataSource = OntarioEnergyBoardDataSource;
//# sourceMappingURL=OntarioEnergyBoardDataSource.js.map